...
export const login = createAsyncThunk('auth/login', async ({ email, password }, thunkAPI) => {
  try {
    ...
    // Lấy addresses và cards từ AsyncStorage
    let addresses = []
    await getUserAddresses(user.email, (result) => { addresses = result }, () => {})
    let cards = []
    await getUserCards(user.email, (result) => { cards = result }, () => {})
...
export const addAddress = createAsyncThunk('auth/addAddress', async ({ email, address }, thunkAPI) => {
  try {
    let newAddress = { ...address, id: Date.now() }
    await insertAddress(email, address, () => {}, () => {})
    return newAddress
  } catch (error) {
    return thunkAPI.rejectWithValue(error.message)
  }
...
export const editAddress = createAsyncThunk('auth/updateAddress', async ({ email, id, address }, thunkAPI) => {
  try {
    await updateAddress(email, id, address, () => {}, () => {});
    return { id, address };
  } catch (error) {
    return thunkAPI.rejectWithValue(error.message);
  }
...
export const removeAddress = createAsyncThunk('auth/deleteAddress', async ({ email, id }, thunkAPI) => {
  try {
...
export const addCard = createAsyncThunk('auth/addCard', async ({ email, card }, thunkAPI) => {
  try {
    let newCard = { ...card, id: Date.now() }
    await insertCard(email, card, () => {}, () => {})
    return newCard
  } catch (error) {
    return thunkAPI.rejectWithValue(error.message)
  }
...
export const removeCard = createAsyncThunk('auth/deleteCard', async ({ email, id }, thunkAPI) => {
  try {
